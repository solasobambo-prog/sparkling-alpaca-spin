...
import { GraduationCap, BookOpen, LayoutDashboard, Settings, LogOut, Sparkles, ChevronRight, ArrowLeft, Library } from 'lucide-react';
...
const Index = () => {
  ...
  const renderContent = () => {
    switch (activeView) {
      case 'quiz':
        return <Quiz moduleTitle={currentLesson.title} onClose={() => setActiveView('overview')} />;
      case 'resources':
        ...
        return (
          <div className="space-y-8">
            ...
            <div className="prose prose-slate max-w-none">
              <h2 className="text-3xl font-bold text-slate-900 mb-6">{currentLesson.title}: Deep Dive</h2>
              <p className="text-lg text-slate-600 leading-relaxed mb-6">
                In this section, we explore the core foundations of the subject. This content is dynamically generated based on your uploaded syllabus <strong>{syllabusName}</strong>.
              </p>
              <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm mb-8">
                <h3 className="text-xl font-bold mb-4">1. Historical Context</h3>
                <p className="text-slate-600 mb-6">
                  Understanding where these concepts originated is crucial for mastering their current application. The evolution of these ideas has been shaped by decades of research and practical refinement.
                </p>
                <h3 className="text-xl font-bold mb-4">2. Core Terminology</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <span className="font-bold text-indigo-600 block mb-1">Term A</span>
                    <p className="text-sm text-slate-500">The primary mechanism for establishing baseline parameters within the framework.</p>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <span className="font-bold text-indigo-600 block mb-1">Term B</span>
                    <p className="text-sm text-slate-500">A secondary process used to validate the outputs of the primary mechanism.</p>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-8">
            <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
              ...
              <div className="pt-8 border-t border-slate-100 flex flex-wrap gap-4">
                <Button 
                  className="bg-indigo-600 hover:bg-indigo-700 px-10 h-12 text-lg shadow-lg shadow-indigo-100"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div 
                className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-indigo-200 transition-colors cursor-pointer group"
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 hidden lg:flex flex-col p-6">
        ...
        <nav className="space-y-2 flex-1">
          <Button 
            variant="ghost" 
            className={cn("w-full justify-start gap-3", activeView === 'overview' ? "text-indigo-600 bg-indigo-50" : "text-slate-600 hover:bg-slate-50")}
          ...
          <div className="max-w-5xl mx-auto flex justify-between items-center">
            <div>
              ...
              <p className="text-slate-500">
                {isReady ? "Your AI-generated curriculum is ready." : "Upload your syllabus to begin."}
              </p>
            </div>
